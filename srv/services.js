...
class ProcessorService extends cds.ApplicationService {
  ...
  async init() {
    //await ca sa asteptam conectiunea la API Business
  ...
  async onCustomerCache(req, next) {
    ...
    if (
      ...
      const customer = await this.S4bupa.transaction(req).send({
        query: SELECT.one(BusinessPartner, (bp) => {
          bp("*"),
  ...
  async onCustomerRead(req) {
    ...
    let result = await this.S4bupa.transaction(req).send({
      query: SELECT.from(BusinessPartner, (bp) => {
        bp("*"),
          bp.addresses((address) => {
            address("email"),
              address.email((emails) => {
